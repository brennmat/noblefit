function X = nf_modelfun_ASW_EApr (varargin)

% X = nf_modelfun_ASW_EApr (T,S,P,A,F,t,tracers)
% X = nf_modelfun_ASW_EApr (info)
%
% Returns gas concentrations given by sum of air saturated water (ASW) and partial-reequilibration excess air (EApr), see OUTPUT as described below.
%
% *** OPTION-A: X = nf_modelfun_ASW_EApr (T,S,P,A,F,t,tracers)
%
% INPUT
% T, S, P,t: see nf_modelfun_ASW
% A, F: see nf_modelfun_EApr
% tracers: list of tracers for which the output should be calculated (cell string)
%
% OUTPUT:
% X: output data (vector of entries as given in 'tracers' at input)
%
% *** OPTION-B: function X = nf_modelfun_ASW_EApr (info)
%
% INPUT:
...
if nargin == 1 % return info
    switch upper(varargin{1})
        ...
        case 'DEFAULTS'
            ...
            Xea  = nf_modelfun_EApr ('DEFAULTS'); % use same defaults as with nf_modelfun_EApr
            X.A  = Xea.A;
            X.F  = Xea.F;

        case 'RANGES'
            X   = nf_modelfun_ASW_EAu ('RANGES');
            Xea = nf_modelfun_EApr ('RANGES');
            X.F = Xea.F;

        case 'TRACERS'
            tASW = nf_modelfun_ASW ('TRACERS');
            tEA  = nf_modelfun_EApr ('TRACERS');
    ...
    for i = 1:length(tracers)
        switch tracers{i}
            ...
            case 'RHe'            
                X(i) = nf_modelfun_ASW_EApr (T,S,P,A,F,t,'He-3') / nf_modelfun_ASW_EApr (T,S,P,A,F,t,'He-4');
                
            case 'RNe'
                X(i) = nf_modelfun_ASW_EApr (T,S,P,A,F,t,'Ne-20') / nf_modelfun_ASW_EApr (T,S,P,A,F,t,'Ne-22');
           
            case 'RAr'
                X(i) = nf_modelfun_ASW_EApr (T,S,P,A,F,t,'Ar-36') / nf_modelfun_ASW_EApr (T,S,P,A,F,t,'Ar-40');

            otherwise % concentrations
                X(i)  = nf_modelfun_ASW (T,S,P,t,tracers{i}) + nf_modelfun_EApr (T,A,F,t,tracers{i});;
